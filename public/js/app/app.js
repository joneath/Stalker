...
Object.append(APP, new Events,new Options, {
  ...
  ,publishNotification: function(message){
    var self = this;
    FB.api('/me/stalker_ed:stalk?location=' + window.location.href + self.socket.socket.sessionid, 'post', function(data){
  ...
  ,plotStalker: function(stalker){
    ...
    this.markers[stalker.id] = this.markers[stalker.id] || new google.maps.Marker({
      map: self.Map,
      icon: new google.maps.MarkerImage('https://graph.facebook.com/' + stalker.id + '/picture')
    });
